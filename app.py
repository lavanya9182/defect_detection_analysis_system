...
# =============================
# SIMPLE CLOUD-SAFE ANOMALY LOGIC
# =============================
def simple_anomaly_score(image: Image.Image):
    """
    Lightweight demo anomaly logic for cloud deployment.
    (PatchCore used offline for research & training)
    """
...
# TABS
# =============================
tab_inference, tab_analytics = st.tabs([
    "🕵️ Inference & Inspection",
    "📊 Analytics & Reporting"
])
...
# =============================
with tab_inference:
    st.subheader("Real-time Capsule Inspection")

    uploaded_file = st.file_uploader(
        "Upload Capsule Image",
        type=["jpg", "png", "jpeg"]
    )
    ...
    if uploaded_file:
        ...
        with col1:
            st.image(image, caption="Uploaded Capsule", use_column_width=True)
        ...
        if st.button("Analyze Image"):
            with st.spinner("Analyzing capsule..."):
                score, heatmap = simple_anomaly_score(image)

                is_defect = score > threshold
                status = "Defect" if is_defect else "Good"
                ...
                defect_type = "General Defect" if is_defect else "Normal (Good)"
                ...
                m4.metric("Defect Type", defect_type)

                with col2:
                    st.subheader("Anomaly Heatmap")
                    st.image(heatmap, use_column_width=True)

                st.toast(f"Result logged (ID: {inspection_id})")

# =============================
# TAB 2: ANALYTICS
# =============================
with tab_analytics:
    st.subheader("📊 QA Analytics Dashboard")
    ...
    if df.empty:
        ...
        a1.metric("Total Inspected", stats["total"])
        a2.metric("Defect Rate", f"{stats['defect_rate']:.1f}%")
        a3.metric("Defects Found", stats["defect_count"])
        a4.metric("Most Frequent Defect", stats["most_frequent_defect"])
        ...
        with c1:
            if chart_status:
                st.pyplot(chart_status)
        with c2:
            if chart_dist:
                st.pyplot(chart_dist)

        st.markdown("### Defect Trends")
        if chart_trend:
            st.pyplot(chart_trend)

        st.markdown("---")
        st.subheader("📄 Generate QA Report")

        if st.button("Generate PDF Report"):
            charts = [chart_status, chart_dist, chart_trend]
            df_defects = df[df["status"] == "Defect"]

            report_file = report.generate_qa_report(
                period, stats, df_defects, charts
            )
